/*!
 * Battery Donut Card — v0.1.5
 * Centered label logic (no label => no gap), YAML-only config
 * MIT License
 */
(() => {
  ...
  window.customCards.push({
    ...
    description: "Smooth battery SoC donut with auto-scaling text.",
    preview: true,
  });
  ...
  class BatteryDonutCard extends HTMLElement {
    ...
    static getStubConfig() {
      return { entity: "sensor.battery_soc", top_label_text: "Battery" };
    }
    static getConfigElement() { return null; } // geen UI-editor

    setConfig(config) {
      if (!config || !config.entity) {
        throw new Error('Set an "entity" (0..100%) in the card config.');
      }
      // Defaults + config (opruiming: verwijderde opties zijn niet meer aanwezig)
      this._config = Object.assign({
        ...
        label_ring_gap: 0,    // alleen toegepast als er labeltekst is
      ...
      if (!this._logged) {
        this._logged = true;
        console.info(`%c ${TAG} %c v${VERSION}`,
    ...
    _render(){
      ...
      Promise.resolve().then(()=> {
        ...
        // ✅ Label aanwezig? gap toepassen. Geen label? geen gap → perfect gecentreerd.
        ...
        const rot = -90; // vast (geen rotate_deg meer)
        const segs= Math.max(12, Number(c.segments || 140));
        const span= (soc/100) * 360;

        // Stops op basis van YAML posities
        ...
        const stops = [
          { pos: 0.00, col: c.color_red    || "#ff0000" },
          ...
          { pos: 1.00, col: c.color_cyan   || "#00bcd4" },
        ];
        ...
        // Actieve boog (segment per segment)
        ...
        // Label boven de ring (alleen als er tekst is)
        if (hasLabel) {
          svg += `
        ...
        // Binnenste teksten — altijd tonen, kWh met 2 dec, % met soc_decimals
        ...
        const style = `
          <style>
            ...
            ha-card {
              ...
              box-shadow:${c.box_shadow}; border:${c.border}; padding:${c.padding};
