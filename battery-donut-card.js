...
(() => {
  ...
  class BatteryDonutCard extends HTMLElement {
    ...
    // Default config shown when user selects the card in the picker
    ...
    // Show mini-editor in the picker
    static getConfigElement() {
      return document.createElement("battery-donut-card-editor");
    }

    // Helps Sections dashboards understand the default grid size
    getGridOptions() {
      return {
        columns: 6,
        rows: 3,
    ...
    _render(){
      ...
      Promise.resolve().then(()=>{
        ...
        let svg=`<svg viewBox="0 0 260 260" xmlns="http://www.w3.org/2000/svg" aria-label="Battery donut">
            <circle cx="${cx}" cy="${cy}" r="${R}" fill="none" stroke="${c.track_color||"#000000"}" stroke-width="${W}" stroke-linecap="round"/>`;
        ...
        svg+=`<g text-anchor="middle" font-family="Inter,system-ui,Segoe UI,Roboto,Arial">
              <text x="${cx}" y="${y_kwh}" font-size="${fs_kwh}" font-weight="300" fill="${innerColor}">${kwh.toFixed(2)} kWh</text>
              <text x="${cx}" y="${y_soc}" font-size="${fs_soc}" font-weight="300" fill="${innerColor}">${soc.toFixed(sd)} %</text>
            </g></svg>`;
        const style=`<style>:host{display:block;width:100%;height:100%;}
            ha-card{background:${c.background};border-radius:${c.border_radius};box-shadow:${c.box_shadow};
              border:${c.border};padding:${c.padding};display:flex;align-items:center;justify-content:center;width:100%;height:100%;}
  ...
  class BatteryDonutCardEditor extends HTMLElement {
    ...
    _render(){
      ...
      this.shadowRoot.innerHTML=`<style>.row{display:flex;gap:12px;align-items:center;margin:8px 0}
        label{width:140px;font-weight:600}input{flex:1;padding:6px 8px;border:1px solid var(--divider-color);
        background:var(--card-background-color);color:var(--primary-text-color);border-radius:6px}</style>
        <div class="row"><label>Entity</label><input id="entity" type="text" placeholder="sensor.battery_soc" value="${c.entity??''}"></div>
        <div class="row"><label>Top label</label><input id="label" type="text" placeholder="Battery" value="${c.top_label_text??'Battery'}"></div>
        <div class="row"><label>Ring radius</label><input id="radius" type="number" min="50" max="200" value="${c.ring_radius??100}"></div>
        <div class="row"><label>Ring width</label><input id="width" type="number" min="4" max="30" value="${c.ring_width??10}"></div>`;
      const emit=()=>this.dispatchEvent(new CustomEvent('config-changed',{detail:{config:{
        ...this._config,
